...
Provisioning Types:
PnP (Plug and Play)
	Can be used for 3CX in the cloud - phones behind a SBC
			3CX on-premise - Phones on local LAN
	Fast and simple
	Admin driven
	Based on multicast messages
		Plug the phone in, it announces that it was been plugged in, PBX receives announcement, admin approves and assigns desktop phone
...
RPS (Direct SIP/Stun)
phones that are not behind a 3CX SBC must be configured with 3CX RPS server
In the 3CX managment console, when youre adding a new phone, need to slect "Direct SIP (STUN - remote)" in the options, and the provisioning link

Legacy phones: Cisco, Polycom and Aastra
Need to wipe phone old firmware and install new 3CX firmware on it
In the managment console, add new phone, select the phone model and provisioning type
...
Need to setup the firewall because the voice call needs to pass throguht it to reach your PBX
Voice calls use serveral port because they use SIP and RTP
...
Firewall ports to be opened
	Ports for SIP Trunk / VOIP Provider
		...
		Port 9000 - 10999 (Inbound, UDP) for RTP
	...
	Ports for 3CX WebMeeting, SMTP ans Activations
		Port 443 (Outound, TCP) to webmeeting.3cx.net
		Port 443 or 5001 (Inbound, TCP) to notify users of incoming web meetings
		Open port you have selected for HTTPS (Inbound, TCP) defauly is 443 or 5001
...
Also you need to disbale SIP ALG (Application Layber Gateway)
...
Reqired to allows connections to 3CX from
	Providers
	3CX apps
	Supported IP Phones
	3CX SBC and Brides
...
This inspects the contenents of the packet and tampers with it uncontrollably
Made for clients not severs
This is why it is recommened to DISABLE SIP ALG
...
These rules process and route the incoming calls to the right destination, like if they go to voicemail, an extention or a queue

Making DID rules
managment console -> Add DID Rule
...
Can make a rule based on the caller ID that is calling you
	Basically the same as DID rules
...
These are virtual numbers assigned directly to the customers physical line
...
Can also route to different destination based on
	Date
	Time of day

Can add all this in the managent console -> inbound rules

Outbound Rules
These dictate how 3CX rotes outgoing calls.
You need to create atleast one outbound rule to start using 3CX 
...
Can manage all this in the managment console -> outbound rules
More info at 
...
Add all of the numbers and set everything up in the managment console -> settings -> PBX -> emergency numbers

Outbound calls drop after 32 seconds by default

VoIP consumes bandwidth, and it is rule of thumb that each VoIP call uses about 30 - 120 kb per second

Firewall checks:
Go to the managment console -> PXB status -> Firewall -> Run
This will run he firewall check and tell you if it passed or not




Holiday Hours Rules
You can make inbound rules based on the date it is, so you can make holiday hours or days rule where all calls will be redirected straight to voicemail
...
These are the menues that you hear when you call a business
...
Here you can add all the the prompts and audio files that the receptionist will use
...
Direct dialing
	When you can call an extention without a menu option
...
Can make a seperate digital receptionist for holidays
...
Works by Admin sends welcome email with the desired 3CX config file, user opens the email and file, 3CX app automatically imports the config

QR Code Provisioning
This is used to provision mobile apps on phones
...
Softphone Mode
	Make and receive calls on my computer without a deskphone
CTI Mode
	A deskphone is required for the app to manage your calls from the desktop

The interface is basically the same for the Mac app and the same for android and apple ios
...
Log into your 3CX managment console, go to Settings -> 3CX Client and put int he server kay and sender ID here, Click OK to save
...
Pre-Recored files are WAV format 8kHz 16bit Mono
They are recored from phone
Can have different greeting depending on the forwarding rule

Voicemail transcription
Translates speech to text
Uses Microsoft Bing Speech API or GCloud Speech API
Requires PRO or Enterptise license
	Transcribed message can be views in email or 3CX web client
...
Voicemail can be disabled
	Has a 4 digit PIn by defualt (Can increase to 10 digit pin)
	3 failed attempt blocks you out for 2 mins

Voicemail is not backed up by default
	Use send email and delete option to reduce backup size
...
You need to first get your API key from your provder, either that be Microsoft Bing Speech API or GCloud speech API
